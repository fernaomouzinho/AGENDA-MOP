...
class AgendaSSOMiddleware(MiddlewareMixin):
    """
    ...
    def attach_user_from_session(self, request):
        ...
        user_id = request.session.get("agenda_user_id")
        print(user_id)
        username = request.session.get("agenda_user")
        print(username)
        roles = request.session.get("agenda_roles", [])
        print(roles)
    ...
    def login_from_token(self, request, token):
        ...
        try:
            ...
            request.session["agenda_user_id"] = user_id
            request.session["agenda_user"] = username
            request.session["agenda_roles"] = roles
            ...
            logger.info("Agenda login success: %s", username)
            return True
